...
module.exports.gradeAnswer = function (args, task_data, callback) {
  ...
  let {c: in_c, m1: in_m1, m2: in_m2, m3: in_m3} = JSON.parse(args.answer.value).answer;


  in_c = canon_input(in_c);
  in_m1 = canon_input(in_m1);
  in_m2 = canon_input(in_m2);
  in_m3 = canon_input(in_m3);


  generateTaskData(args.task)
    .then(function ({privateData}) {
      let [ex_c, ex_m1, ex_m2, ex_m3] = privateData.answer.split("\n");
      ...
      if (in_m1 === ex_m1 && in_m2 === ex_m2 && in_m3 === ex_m3) {
        score += 50;
        metalsOK = true;
      }
      ...
      const message = cityOK
        ? metalsOK
          ? `Vous avez atteint le score maximum que vous pouvez obtenir à
             cette épreuve, compte tenu des indices que vous avez obtenus.`
          : "Le lieu est le bon, mais au moins un des trois métaux est faux."
        : metalsOK
          ? "Les trois métaux sont les bons, mais le lieu est faux."
          : "Ni le lieu ni les métaux ne sont les bons.";
